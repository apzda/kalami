 ...
 */
@Slf4j
public class ResizePlugin extends AbstractPlugin<Map<String, String>> {
    ...
    @Override
    public UploadedFile alter(UploadedFile file, String path, OssBackend ossBackend) throws Exception {
        val ext = file.getExt();
        if (!supported(ext)) {
        ...
        val props = mapConfig;
        val width = props.getInt(PROP_WIDTH_I);
        val height = props.getInt(PROP_HEIGHT_I);
        if (width == 0 && height == 0) {
            throw new IllegalArgumentException("width or height cannot be both 0");
        }
        ...
        graphics.dispose();
        val tmpImg = FileUtil
            .createTempFile("resize_", "." + file.getExt(), new File(props.getString(PROP_TMPDIR_S)).toPath())
            .toAbsolutePath()
            .toString();
