 ...
 **/
@Slf4j
public class StorageFile implements IOssFile {
    ...
    public StorageFile(String backend, String filePath) {
        ...
        try {
            OssBackend ossBackend = StorageManager.getOssBackend(backend);
            ossFile = ossBackend.getFile(this.filePath);
        }
        catch (IOException e) {
    ...
    public static Optional<OssBackend> getDefaultBackend() {
        try {
            val ossBackend = StorageManager.getOssBackend();
