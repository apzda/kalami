 ...
 */
package com.apzda.hajimi.weixin.http;
...
@Slf4j
@Getter
public class WxCommonRequestWrapper extends HttpRequestWrapper {
    ...
    public static WxCommonRequestWrapper from(@Nonnull ServerRequest request) {
        val wrapper = request.attribute(HttpRequestWrapper.BROKER_REQUEST_WRAPPER).orElse(null);
        ...
        request.attributes().put(HttpRequestWrapper.BROKER_REQUEST_WRAPPER, requestWrapper);
    ...
    public boolean checkAuthentication(WxConfiguration config) throws Exception {
        val secret = getSecret();
        val appid = getAppid();

        if (!StringUtils.isAllBlank(secret, appid)) {
            return Objects.equals(secret, config.getSecret()) && Objects.equals(appid, config.getAppid());
        }
