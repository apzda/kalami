 ...
 */
package com.apzda.hajimi.weixin.mp.handler;
...
public class WxMpCallbackHandlerFunction implements HandlerFunction<ServerResponse> {

    private final WxMpService wxMpService;

    private final ObjectProvider<IWxMpNotificationHandler> handlerProvider;
    ...
    public ServerResponse handle(@Nonnull ServerRequest request) throws Exception {
        ...
        try {
            val service = wxMpService.switchoverTo(appId);
            ...
            if (method == HttpMethod.GET) {
                ...
                if (encryptType == null) {
                    ...
                    // aes加密的消息
                    inMessage = WxMpXmlMessage.fromEncryptedXml(requestBody, service.getWxMpConfigStorage(), timestamp,
                            nonce, msgSignature);
                    if (log.isTraceEnabled()) {
                        log.trace("\n消息解密后内容为[{}]：\n{} ", appId, inMessage.toString());
                    }
    ...
    @Nullable
    protected ServerResponse notify(String appId, @Nullable WxMpXmlMessage response, String body,
        ...
        if (!CollectionUtils.isEmpty(handlers)) {
            for (IWxMpNotificationHandler handler : handlers) {
                try {
