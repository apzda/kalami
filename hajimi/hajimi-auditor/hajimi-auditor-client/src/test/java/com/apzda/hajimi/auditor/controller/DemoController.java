 ...
 */
package com.apzda.hajimi.auditor.controller;
...
public class DemoController {
    ...
    @Audit(activity = "test", message = "#{'you are get then id is: ' + #id +', then result is:' + #returnObj }")
    @GetMapping("/audit/{id}")
    public String shouldBeAudited(@PathVariable String id) {
        return shouldBeAudited1(id);
    }

    @Audit(activity = "test", message = "#{'new = '+ #newValue}")
    ...
    @Audit(activity = "test", message = "#{'old = ' + #oldValue}")
