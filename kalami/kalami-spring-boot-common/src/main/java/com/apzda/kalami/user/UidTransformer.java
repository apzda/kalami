 ...
 * @version 1.0.0
 */
public class UidTransformer implements Transformer<Serializable> {

    private final Lazy<UserInfoService> userInfoService;

    public UidTransformer() {
    ...
    public Object transform(Serializable value, boolean all) {
        ...
        try {
            val userInfo = service.getUserInfo(value);
            if (userInfo == null) {
                return null;
            }
            if (all) {
    ...
    public static class DefaultUserInfoService implements UserInfoService {
