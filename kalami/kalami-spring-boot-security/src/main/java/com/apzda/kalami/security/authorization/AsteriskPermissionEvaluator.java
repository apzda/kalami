...
public class AsteriskPermissionEvaluator implements PermissionEvaluator {
    ...
    public AsteriskPermissionEvaluator(@Nonnull ObjectProvider<PermissionChecker> checkerProvider) {
        this.checkers = checkerProvider.orderedStream().toList();
        for (val checker : checkers) {
            log.debug("AsteriskPermissionEvaluator: checker '{}' configured for use",
