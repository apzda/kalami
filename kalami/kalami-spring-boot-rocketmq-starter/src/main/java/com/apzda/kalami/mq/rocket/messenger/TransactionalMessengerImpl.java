 ...
 */
@Slf4j
public class TransactionalMessengerImpl implements IMessenger, ApplicationListener<ApplicationReadyEvent> {
    ...
    private final RocketMqConfigProperties properties;
    ...
    public TransactionalMessengerImpl(RocketMqConfigProperties properties, Producer producer, ObjectMapper objectMapper,
    ...
    @Slf4j
    private record MailSender(Producer producer, IMailboxService mailboxService, RocketMqConfigProperties postmanConfig,
        ...
        public void post(@Nonnull Mailbox trans) {
            try {
                Optional.ofNullable(trans.getProperties()).ifPresent(p -> {
                    val tid = p.get("_tid");
