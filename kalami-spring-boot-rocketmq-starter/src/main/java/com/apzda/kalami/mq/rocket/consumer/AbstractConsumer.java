...
import com.apzda.kalami.mq.rocket.listener.MessageListenerDelegate;
...
public abstract class AbstractConsumer implements ApplicationListener<ApplicationReadyEvent>, MessageListener {
    ...
    private final List<RocketMqConfigProperties.TopicConfig> topics;
    ...
    private final RocketMqConfigProperties.ConsumerConfig config;
    ...
    public AbstractConsumer(@Nonnull ClientServiceProvider clientServiceProvider,
        ...
        this.enabled = config.isEnabled() && !CollectionUtils.isEmpty(topics)
                && topics.stream().anyMatch(RocketMqConfigProperties.TopicConfig::isEnable);
        this.config = config;
    }
        ...
        if (this.consumer != null) {
            ...
            try {
                if (!CollectionUtils.isEmpty(this.topics)) {
                    for (RocketMqConfigProperties.TopicConfig topic : this.topics) {
                        try {
                            if (topic.isEnable()) {
    ...
    public ConsumeResult consume(@Nonnull MessageView messageView) {
        ...
        val tid = messageView.getProperties().get("_tid");
